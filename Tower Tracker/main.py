...
class FivePageApp:
    ...
    def create_home_page(self):
        title_label = tk.Label(self.root, text='Welcome to Cellular Tower Project Management', font=('Helvetica', 16, 'bold'), bg='light blue')
        title_label.pack(pady=20)

        tk.Button(self.root, text='Login', command=lambda: self.show_page(1)).pack(pady=10)
        tk.Button(self.root, text='Exit', command=self.root.destroy).pack(pady=10)

    def create_login_page(self):
        title_label = tk.Label(self.root, text='LOGIN', font=('Helvetica', 16, 'bold'), bg='light blue')
        title_label.pack(pady=20)
